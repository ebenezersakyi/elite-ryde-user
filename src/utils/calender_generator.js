...
export const genetate = (
  ...
  for (let i = 0; i < firstDateOfMonth.day(); ++i) {
    arrayofDates.push({
      ...
      currentMonth: false,
      pastMonth: true,
      past: dayjs(firstDateOfMonth.date(i).toDate().toDateString()).isBefore(
        start
      ),
      blocked: isDateBlocked(firstDateOfMonth.date(i))
    });
  }
  ...
  for (
    ...
    arrayofDates.push({
      date: lastDayOfMonth.date(i),
      currentMonth: false,
      past: dayjs(lastDayOfMonth.date(i).toDate().toDateString()).isBefore(
        start
      ),
      blocked: isDateBlocked(lastDayOfMonth.date(i))
    });
  }
  ...
  function isDateBlocked(date) {
    for (const [a, b] of blockedDates) {
      if (date.isAfter(a) && date.isBefore(b)) {
        return true;
      }
    }
    return false;
