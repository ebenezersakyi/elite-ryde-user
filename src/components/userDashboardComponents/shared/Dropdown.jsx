...
const Dropdown = ({category, options}) => {
  ...
  return (
    <div className="min-w-[100px]">
      <h4 className="text-[1.2rem] font-medium mb-4">{category}:</h4>
      <div
      ...
      >
        ...
        {/* options */}
        {show && (
          <div className="absolute top-[100%] z-[100] bg-[#000] py-4 mt-2 backdrop-blur-[12px] rounded-md border-[#fff] border-[1px]  w-fit left-0 flex flex-col px-2">
            {options.map((elem, inx) => {
              return (
                <p
                  className="cursor-pointer capitalize text-[1rem] w-fit p-2"
                  key={inx}
                  onClick={() => {
                    setSelected(elem);
                  }}
